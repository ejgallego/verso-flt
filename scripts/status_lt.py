...
def summarize(path: Path, warn_below: float) -> str:
    ...
    drift = sum(score.exact_drift_count > 0 for score in scores)
    ...
    review_refs = sum(score.ref_hint_count > 0 for score in scores)
    ...
    return (
        ...
        f"low={low} drift={drift} "
        f"pure_metadata={pure_metadata} reground={reground} witness={witness} "
        f"review_refs={review_refs} strong_refs={strong_refs} env_ref_hints={env_refs}"
    )
