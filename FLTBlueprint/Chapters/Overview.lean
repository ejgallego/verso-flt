...
representation becomes modular in the quaternionic sense.
:::
...
case via Skinner--Wiles style arguments.
:::
...
Brauer's theorem tricks, and the move to a 3-adic specialization.
:::
...
The contradiction uses {uses "odlyzko_root_discriminant_bound"}[] together with
the deformation-theoretic bridge recorded earlier in
{uses "hardly_ramified_compatible_family"}[].
:::
