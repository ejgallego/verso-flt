...
After this chapter, you should next read {ref "first_reductions"}[the first reductions chapter], which explains
...
{ref "overview_strategy"}[the overview chapter] is an extremely sketchy overview of how the rest of the
...
The next chapter to read, where the proof begins, is {ref "first_reductions"}[the first reductions chapter].
